...
function footer ({ tools = [] } = {}) {
  const element = bel`<div class="tools">
    <h3> tools </h3>
    <ul class="chapter-list">${
      tools.map(({ title, url }) => bel`<li class="chapter"><a href="${url}">${title}</a></li>`)
    }</ul>
  </div>`
  return element
...
function header ({
  ...
  const element = bel`<div class="header">
    <a href="${home}">
...
},{"bel":3,"csjs-inject":6}],28:[function(require,module,exports){
const bel = require('bel')
const csjs = require('csjs-inject')
...
},{"bel":3,"csjs-inject":6,"footer":26,"header":27}]},{},[1]);
